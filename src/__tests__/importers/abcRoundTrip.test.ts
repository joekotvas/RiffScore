...
/** 'F#4:q.' → a quarter, dotted; a trailing '-' ties; '!' forces the accidental glyph. */
const ev = (spec: Spec, tuplet?: ScoreEvent['tuplet']): ScoreEvent => {
  const parse = (s: string) => {
    const m = s.match(/^([^:]+?)(-)?(!)?(?::([a-z0-9]+?)(\.)?)?$/)!;
    return { pitch: m[1], tied: !!m[2], show: !!m[3], duration: DUR[m[4] ?? 'q'], dotted: !!m[5] };
  };
  ...
  return {
    ...
    notes: specs.map((s) => ({
      ...
      pitch: s.pitch,
      ...(s.tied ? { tied: true } : {}),
      ...(s.show ? { accidentalDisplay: 'show' as const } : {}),
    })),
    ...(tuplet ? { tuplet } : {}),
  };
};
