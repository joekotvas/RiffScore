...
const MockNoteTrigger = () => {
    const { handleNoteSelection, selection, setSelection, transposeSelection } = require('../context/ScoreContext').useScoreContext();

    return (
        <div>
            <div data-testid="selection-count">{selection.selectedNotes ? selection.selectedNotes.length : 0}</div>
            <div data-testid="selected-note-ids">{selection.selectedNotes ? selection.selectedNotes.map((n: any) => n.noteId).join(',') : ''}</div>
            <button 
            ...
            <button
                ...
                onClick={() => transposeSelection('up', false)}
            />
        </div>
    );
};
